...
# Example: top 5 CBSAs by total permits (2010–2019)
PERMITS |>
...
# Plot: HG vs ΔRent Burden
ggplot(summary_cbsa, aes(x = HG_avg, y = rb_end - rb_start)) +
  geom_hline(yintercept = 0, linetype = "dotted") +
  geom_vline(xintercept = 100, linetype = "dotted") +
  geom_point(alpha = 0.6) +
  labs(title = "Housing Growth vs Change in Rent Burden (2014→2023)",
...
#Co-Sponsor Metro: Miami–Fort Lauderdale, FL Metro Area

##Housing affordability is a growing challenge across the U.S. Our analysis of ACS (2009–2023) and BLS QCEW data shows that metros that permit more housing experience lower rent burden and continued population growth, while low-building metros face rising rent pressure.
...
#Burlington improved from 112 → 90.5

#Miami worsened from 136 → 144
...
#Rent burden dropped significantly (112 → 90.5)
...
#❌ When Housing Doesn’t Keep Up

#Miami–Fort Lauderdale:

#Rent burden worsened (136 → 144)

#Population grew +253,380

#Housing growth very low (HG = 35.8)

#High demand, insufficient supply → higher rents.

#👥 Who Benefits

#Health care workers (NAICS 62)
#Large share of workforce in both metros — housing affordability improves staffing stability.

#Finance & insurance workers (NAICS 52)
#Common in Florida and North Carolina — lower rent frees income for saving and spending, supporting the local economy.
...
#Prioritize infill, duplex/quadplex, and mid-rise “missing middle” housing

#📌 Bottom Line

#Burlington shows building works.
#Miami shows what happens when it doesn’t.

#Build more homes → lower rents → stronger workforce → thriving communities.
options(STA9750.GITHUB_ID = "Sakib49")

#task -5 visualisation
#| label: fig-hg-scatter
#| fig-cap: "Housing Growth — Instantaneous vs Rate-Based (color = Composite)"
#| fig-width: 7
#| fig-height: 5
...
latest <- max(HG$year, na.rm = TRUE)

hg_latest <- HG %>%
  filter(year == latest, is.finite(HG_inst), is.finite(HG_rate)) %>%
  # keep names & composite
  select(NAME, CBSA, year, HG_inst, HG_rate, HG_Composite)

ggplot(hg_latest, aes(x = HG_inst, y = HG_rate, color = HG_Composite)) +
  geom_hline(yintercept = 0, linetype = "dotted") +
  geom_vline(xintercept = 0, linetype = "dotted") +
  geom_point(alpha = 0.7, size = 2) +
  scale_color_viridis_c(option = "plasma", name = "HG Composite") +
  scale_x_continuous("Permits per 1,000 residents (instant)", labels = label_number(accuracy = 0.1)) +
  scale_y_continuous("Permits per 1,000 net new residents (5-yr)", labels = label_number(accuracy = 1)) +
  labs(title = paste0("Housing Growth — Instant vs Rate (", latest, ")"),
       caption = "Instant = permits / population; Rate = permits / 5-year population growth") +
  theme_minimal(base_size = 12)

#| label: fig-hg-topbars
#| fig-cap: "Top 20 CBSAs by Housing Growth Composite (latest year)"
#| fig-width: 7
#| fig-height: 7

hg_top <- HG %>%
  filter(year == latest) %>%
  arrange(desc(HG_Composite)) %>%
  slice_head(n = 20) %>%
  mutate(NAME = reorder(NAME, HG_Composite))

ggplot(hg_top, aes(x = HG_Composite, y = NAME)) +
  geom_col() +
  geom_text(aes(label = round(HG_Composite, 1)),
            hjust = -0.1, size = 3.2) +
  scale_x_continuous("HG Composite (100 = U.S. avg)", limits = c(0, max(hg_top$HG_Composite)*1.15)) +
  ylab(NULL) +
  labs(title = paste0("Top 20 YIMBY CBSAs by Composite (", latest, ")"),
       caption = "Composite = average of standardized instant and rate metrics") +
  theme_minimal(base_size = 12)

#| label: fig-hg-trends
#| fig-cap: "Housing Growth Trends (Instant, Rate, Composite) — Selected CBSAs"
#| fig-width: 7
#| fig-height: 6

focus_cities <- c("Burlington, NC Metro Area",
                  "Miami-Fort Lauderdale-Pompano Beach, FL Metro Area")  # adjust if your NAME strings differ

hg_focus <- HG %>%
  filter(NAME %in% focus_cities) %>%
  select(NAME, year, HG_inst, HG_rate, HG_Composite) %>%
  tidyr::pivot_longer(cols = c(HG_inst, HG_rate, HG_Composite),
                      names_to = "metric", values_to = "value") %>%
  mutate(metric = dplyr::recode(metric,
                                HG_inst = "Instant (per 1k residents)",
                                HG_rate = "Rate (per 1k net new)",
                                HG_Composite = "Composite (index)"))

ggplot(hg_focus, aes(x = year, y = value, color = NAME)) +
  geom_line(size = 1) +
  facet_wrap(~ metric, scales = "free_y") +
  scale_x_continuous(breaks = seq(min(hg_focus$year), max(hg_focus$year), by = 2)) +
  labs(title = "Housing Growth Over Time — Burlington vs. Miami",
       x = NULL, y = NULL, color = "CBSA",
       caption = "Instant = permits/pop; Rate = permits/5-year pop growth; Composite = standardized average") +
  theme_minimal(base_size = 12) +
  theme(legend.position = "bottom")

#| label: fig-hg-scatter-labeled
#| fig-cap: "Instant vs Rate (labeled): Burlington & Miami"
#| fig-width: 7
#| fig-height: 5

label_rows <- hg_latest %>% filter(NAME %in% focus_cities)

ggplot(hg_latest, aes(x = HG_inst, y = HG_rate, color = HG_Composite)) +
  geom_point(alpha = 0.6) +
  ggrepel::geom_text_repel(data = label_rows, aes(label = NAME),
                           size = 3.2, fontface = "bold", box.padding = 0.3) +
  scale_color_viridis_c(option = "plasma", name = "HG Composite") +
  scale_x_continuous("Permits per 1,000 residents (instant)", labels = label_number(accuracy = 0.1)) +
  scale_y_continuous("Permits per 1,000 net new residents (5-yr)", labels = label_number(accuracy = 1)) +
  labs(title = paste0("Housing Growth — Instant vs Rate (", latest, ")"),
       caption = "Highlighted: Burlington, NC; Miami–Fort Lauderdale, FL") +
  theme_minimal(base_size = 12)
...
Housing Affordability & Growth Report

(ACS + Census Building Permits, 2009–2023)**

Executive Summary

This report analyzes U.S. housing affordability, population dynamics, and construction patterns from 2009–2023 using ACS 1-year estimates and Census building permits. We construct two custom indicators:

Rent Burden Index (RBI)
(100 = baseline affordability; >100 = more burden)
Measures how rent affordability changed over time.

Housing Growth Composite Index (HG)
(100 = national average)
Combines permitting per capita and permitting relative to population growth to identify “YIMBY metros” — regions building enough housing to meet demand.

Key Findings

Metros with higher permit activity experienced slower rent-burden growth or declines.

Metros with low housing supply response faced sharply rising rent burden.

Burlington, NC stands out as a YIMBY success — rising population with declining rent burden.

Miami-Fort Lauderdale represents housing supply failure — strong population inflows, but insufficient housing supply → rapidly worsening rent pressure.

Household size did not meaningfully increase, meaning overcrowding is not masking demand — supply truly fell behind in constrained markets.

Across metros, median rent remains tightly correlated with median income, indicating standard economic forces still dominate regional housing markets.
...
Census Building Permits Survey (2009–2023)

BLS QCEW industry employment data (for workforce impact analysis)

Key Metrics Constructed

Rent Burden Index (RBI)

RBI=Median RentMedian Income×100
RBI=
Median Income
Median Rent
	​
...
Housing Growth Composite

Instant permits per capita

Permits relative to 5-year population growth

Standardized + averaged

YIMBY classification
A metro is a “YIMBY success” if it:
✅ had high rent burden early
✅ reduced rent burden
✅ grew population
✅ built above the national average
(Burlington, NC qualifies)

Visual Findings & Interpretation
1️⃣ Rent & Income Relationship (Scatterplot, 2009)

A clear positive correlation — higher income metros pay more rent.
This validates economic housing theory and confirms ACS reliability.

2️⃣ Housing Permits Over Time (line plot)

We see a strong post-2011 recovery in permitting nationally, but uneven across regions.

Top builders: Houston, Dallas, NYC, Atlanta, DC
Interpretation: Sun Belt and large metros have maintained strong supply pipelines.

3️⃣ Burlington vs Miami Housing Growth (3-panel time series)
Metric	Burlington, NC	Miami–Fort Lauderdale
Housing growth	Consistent, near nat'l avg	Volatile, often below demand
Population	Moderate growth	Very high growth
Rent burden	Fell (112 → 90.5)	Rose sharply (136 → 144)
